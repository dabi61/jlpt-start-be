"""Import JLPT N5 dataset from JSON files and upload media to Cloudflare R2."""
...
class Command(BaseCommand):
    help = 'Import N5 JSON dataset, upload media to R2, and map media URLs into question data.'
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            type=str,
            default='n5',
            help='Source directory containing N5 JSON and media folders (default: n5).',
        )
        parser.add_argument(
            '--prefix',
            type=str,
            default='n5/images',
            help='R2 key prefix for uploaded media (default: n5/images).',
        )
        parser.add_argument(
            '--clear',
            action='store_true',
            help='Clear existing N5 tables before importing.',
        )
        parser.add_argument(
            '--skip-upload-local',
            action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS('N5 import finished.'))
        ...
        for idx, path in enumerate(sorted(media_files), 1):
            ...
            if dry_run:
                ...
                self._upsert_media_asset(
                    source_type=N5MediaAsset.SourceType.LOCAL,
    ...
    def _detect_media_type(ext: str) -> str:
        ...
        if ext in {'.png', '.jpg', '.jpeg', '.gif'}:
            return N5MediaAsset.MediaType.IMAGE
        if ext in {'.mp3', '.wav', '.m4a', '.ogg'}:
            return N5MediaAsset.MediaType.AUDIO
        return N5MediaAsset.MediaType.OTHER
    ...
    def _upsert_media_asset(
        ...
        *,
        ...
        N5MediaAsset.objects.update_or_create(
    ...
    def _import_one_json(
        ...
        *,
        ...
        with transaction.atomic():
            section, _ = N5Section.objects.get_or_create(
            ...
            subcategory, _ = N5Subcategory.objects.get_or_create(
                section=section,
            ...
            exam, created = N5Exam.objects.update_or_create(
            ...
            for order, row in enumerate(rows):
                ...
                question_defaults = {
                    ...
                    'jlpt_level': int(row.get('level') or 5),
                ...
                question, q_created = N5Question.objects.update_or_create(
                    exam=exam,
                    source_id=source_id,
                ...
                for item_order, content in enumerate(content_rows):
                    ...
                    item, item_created = N5QuestionItem.objects.update_or_create(
                        question=question,
    ...
    def _subcategory_sort_order(section_key: str, subcategory_key: str) -> int:
        section_order = {
            'TuVung': ['CachDocKanji', 'DongNghia', 'BieuHienTu', 'CachDocHiragana'],
    ...
    def _download_and_upload_remote_url(self, *, value: str, uploader: R2Uploader) -> str:
        try:
            ...
            self._upsert_media_asset(
                source_type=N5MediaAsset.SourceType.REMOTE,
                media_type=media_type,
                source_url=value,
                source_path='',

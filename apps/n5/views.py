...
from .serializers import (
    N5SectionSerializer,
    N5SubcategorySerializer,
    N5ExamSerializer,
    N5QuestionListSerializer,
    N5QuestionSerializer,
    N5QuestionItemSerializer,
    N5MediaAssetSerializer,
)


class N5SectionViewSet(viewsets.ModelViewSet):
    queryset = N5Section.objects.all()
    serializer_class = N5SectionSerializer
...
class N5SubcategoryViewSet(viewsets.ModelViewSet):
    queryset = N5Subcategory.objects.select_related('section').all()
    serializer_class = N5SubcategorySerializer
    pagination_class = StandardResultsSetPagination
    permission_classes = [IsAdminOrReadOnly]
    filter_backends = [filters.SearchFilter, filters.OrderingFilter]
...
class N5ExamViewSet(viewsets.ModelViewSet):
    queryset = N5Exam.objects.select_related('subcategory', 'subcategory__section').all()
    serializer_class = N5ExamSerializer
    pagination_class = StandardResultsSetPagination
    permission_classes = [IsAdminOrReadOnly]
    filter_backends = [filters.SearchFilter, filters.OrderingFilter]
    ...
    @extend_schema(responses={200: N5QuestionListSerializer(many=True)})
    @action(detail=True, methods=['get'], filter_backends=[])
    def questions(self, request, pk=None):
        exam = self.get_object()
        queryset = exam.questions.order_by('display_order', 'id')
        page = self.paginate_queryset(queryset)
        serializer = N5QuestionListSerializer(page if page is not None else queryset, many=True)
        if page is not None:
            return self.get_paginated_response(serializer.data)
        return Response(serializer.data)


class N5QuestionViewSet(viewsets.ModelViewSet):
    queryset = N5Question.objects.select_related('exam', 'exam__subcategory', 'exam__subcategory__section').all()
    pagination_class = StandardResultsSetPagination
    permission_classes = [IsAdminOrReadOnly]
    filter_backends = [filters.SearchFilter, filters.OrderingFilter]
    ...
    def get_serializer_class(self):
        if self.action == 'list':
            return N5QuestionListSerializer
        return N5QuestionSerializer
    ...
    @extend_schema(responses={200: N5QuestionItemSerializer(many=True)})
    ...
    def items(self, request, pk=None):
        ...
        serializer = N5QuestionItemSerializer(queryset, many=True)
        return Response(serializer.data)


class N5QuestionItemViewSet(viewsets.ModelViewSet):
    queryset = N5QuestionItem.objects.select_related(
        'question', 'question__exam', 'question__exam__subcategory', 'question__exam__subcategory__section'
    ).all()
    serializer_class = N5QuestionItemSerializer
    pagination_class = StandardResultsSetPagination
    permission_classes = [IsAdminOrReadOnly]
    filter_backends = [filters.SearchFilter, filters.OrderingFilter]
...
class N5MediaAssetViewSet(viewsets.ModelViewSet):
    queryset = N5MediaAsset.objects.all()
    serializer_class = N5MediaAssetSerializer
    pagination_class = StandardResultsSetPagination
    permission_classes = [IsAdminOrReadOnly]
    filter_backends = [filters.SearchFilter, filters.OrderingFilter]

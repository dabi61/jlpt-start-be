...
from .serializers import (
    N2SectionSerializer,
    N2SubcategorySerializer,
    N2ExamSerializer,
    N2QuestionListSerializer,
    N2QuestionSerializer,
    N2QuestionItemSerializer,
    N2MediaAssetSerializer,
)


class N2SectionViewSet(viewsets.ModelViewSet):
    queryset = N2Section.objects.all()
    serializer_class = N2SectionSerializer
...
class N2SubcategoryViewSet(viewsets.ModelViewSet):
    queryset = N2Subcategory.objects.select_related('section').all()
    serializer_class = N2SubcategorySerializer
    pagination_class = StandardResultsSetPagination
    permission_classes = [IsAuthenticated]
    filter_backends = [filters.SearchFilter, filters.OrderingFilter]
...
class N2ExamViewSet(viewsets.ModelViewSet):
    queryset = N2Exam.objects.select_related('subcategory', 'subcategory__section').all()
    serializer_class = N2ExamSerializer
    pagination_class = StandardResultsSetPagination
    permission_classes = [IsAuthenticated]
    filter_backends = [filters.SearchFilter, filters.OrderingFilter]
    ...
    @extend_schema(responses={200: N2QuestionListSerializer(many=True)})
    @action(detail=True, methods=['get'])
    def questions(self, request, pk=None):
        exam = self.get_object()
        queryset = exam.questions.order_by('display_order', 'id')
        page = self.paginate_queryset(queryset)
        serializer = N2QuestionListSerializer(page if page is not None else queryset, many=True)
        if page is not None:
            return self.get_paginated_response(serializer.data)
        return Response(serializer.data)


class N2QuestionViewSet(viewsets.ModelViewSet):
    queryset = N2Question.objects.select_related('exam', 'exam__subcategory', 'exam__subcategory__section').all()
    pagination_class = StandardResultsSetPagination
    permission_classes = [IsAuthenticated]
    filter_backends = [filters.SearchFilter, filters.OrderingFilter]
    ...
    def get_serializer_class(self):
        if self.action == 'list':
            return N2QuestionListSerializer
        return N2QuestionSerializer
    ...
    @extend_schema(responses={200: N2QuestionItemSerializer(many=True)})
    @action(detail=True, methods=['get'], pagination_class=None)
    def items(self, request, pk=None):
        question = self.get_object()
        queryset = question.items.order_by('item_order', 'id')
        serializer = N2QuestionItemSerializer(queryset, many=True)
        return Response(serializer.data)


class N2QuestionItemViewSet(viewsets.ModelViewSet):
    queryset = N2QuestionItem.objects.select_related(
        'question', 'question__exam', 'question__exam__subcategory', 'question__exam__subcategory__section'
    ).all()
    serializer_class = N2QuestionItemSerializer
    pagination_class = StandardResultsSetPagination
    permission_classes = [IsAuthenticated]
    filter_backends = [filters.SearchFilter, filters.OrderingFilter]
...
class N2MediaAssetViewSet(viewsets.ModelViewSet):
    queryset = N2MediaAsset.objects.all()
    serializer_class = N2MediaAssetSerializer
    pagination_class = StandardResultsSetPagination
    permission_classes = [IsAuthenticated]
    filter_backends = [filters.SearchFilter, filters.OrderingFilter]

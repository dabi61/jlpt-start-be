"""Import JLPT N1 dataset from JSON files and upload media to Cloudflare R2."""
...
class Command(BaseCommand):
    help = 'Import N1 JSON dataset, upload media to R2, and map media URLs into question data.'
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            type=str,
            default='n1',
            help='Source directory containing N1 JSON and media folders (default: n1).',
        )
        parser.add_argument(
            '--prefix',
            type=str,
            default='n1/images',
            help='R2 key prefix for uploaded media (default: n1/images).',
        )
        parser.add_argument(
            '--clear',
            action='store_true',
            help='Clear existing N1 tables before importing.',
        )
        parser.add_argument(
            '--skip-upload-local',
            action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        if clear and not dry_run:
            self.stdout.write(self.style.WARNING('Clearing existing N1 dataset...'))
        ...
        for json_path in json_files:
            ...
            if skip_existing_exams and not dry_run:
                if N1Exam.objects.filter(source_file=str(relative)).exists():
        ...
        self.stdout.write(self.style.SUCCESS('N1 import finished.'))
        for key in sorted(summary.keys()):
            self.stdout.write(f"  {key}: {summary[key]}")

    def _clear_existing_data(self):
        N1Section.objects.all().delete()
        N1MediaAsset.objects.all().delete()
    ...
    def _load_remote_url_cache(self) -> dict[str, str]:
        ...
        for source_url, public_url in (
            N1MediaAsset.objects
            .filter(source_type=N1MediaAsset.SourceType.REMOTE)
            .exclude(source_url='')
            .exclude(public_url='')
        ...
        for idx, path in enumerate(sorted(media_files), 1):
            ...
            if dry_run:
                ...
                self._upsert_media_asset(
                    source_type=N1MediaAsset.SourceType.LOCAL,
    ...
    def _detect_media_type(ext: str) -> str:
        ...
        if ext in {'.png', '.jpg', '.jpeg', '.gif'}:
            return N1MediaAsset.MediaType.IMAGE
        if ext in {'.mp3', '.wav', '.m4a', '.ogg'}:
            return N1MediaAsset.MediaType.AUDIO
        return N1MediaAsset.MediaType.OTHER
    ...
    def _upsert_media_asset(
        ...
        *,
        ...
        N1MediaAsset.objects.update_or_create(
    ...
    def _import_one_json(
        ...
        *,
        ...
        if rows and isinstance(rows[0], dict):
            try:
                dataset_level = int(rows[0].get('level') or 1)
        ...
        with transaction.atomic():
            section, _ = N1Section.objects.get_or_create(
            ...
            subcategory, _ = N1Subcategory.objects.get_or_create(
                section=section,
            ...
            exam, created = N1Exam.objects.update_or_create(
            ...
            for order, row in enumerate(rows):
                ...
                question_defaults = {
                    ...
                    'jlpt_level': int(row.get('level') or 1),
                ...
                question, q_created = N1Question.objects.update_or_create(
                    exam=exam,
                    source_id=source_id,
                ...
                for item_order, content in enumerate(content_rows):
                    ...
                    item, item_created = N1QuestionItem.objects.update_or_create(
                        question=question,
    ...
    def _subcategory_sort_order(section_key: str, subcategory_key: str) -> int:
        section_order = {
            'TuVung': ['CachDocKanji', 'DongNghia', 'BieuHienTu', 'CachDungTu'],
            'NguPhap': ['DangNguPhap', 'ThanhLapCau', 'NguPhapTheoDoanVan'],
            'Doc': ['DoanVanNgan', 'DoanVanTrungBinh', 'DoanVanDai', 'DocHieuChuDe', 'TimThongTin', 'DocHieuTongHop'],
    ...
    def _download_and_upload_remote_url(self, *, value: str, uploader: R2Uploader) -> str:
        try:
            ...
            self._upsert_media_asset(
                source_type=N1MediaAsset.SourceType.REMOTE,
                media_type=media_type,
                source_url=value,
                source_path='',

"""Serializers for N1 practice data APIs."""
...
class N1SectionSerializer(serializers.ModelSerializer):
    class Meta:
        model = N1Section
        fields = [
            'id',
...
class N1SubcategorySerializer(serializers.ModelSerializer):
...
class N1ExamSerializer(serializers.ModelSerializer):
...
class N1QuestionItemSerializer(serializers.ModelSerializer):
    class Meta:
        model = N1QuestionItem
        fields = [
            'id',
            'question',
            'item_order',
...
class N1QuestionListSerializer(serializers.ModelSerializer):
...
class N1QuestionSerializer(serializers.ModelSerializer):
    items = N1QuestionItemSerializer(many=True, read_only=True)
    exam_name = serializers.CharField(source='exam.name', read_only=True)
    section_code = serializers.CharField(source='exam.subcategory.section.code', read_only=True)
    subcategory_code = serializers.CharField(source='exam.subcategory.code', read_only=True)
...
class N1MediaAssetSerializer(serializers.ModelSerializer):
    class Meta:
        model = N1MediaAsset
        fields = [
            'id',
